...
class TeamMemberController extends Controller
{
    ...
    /**
     * قائمة أعضاء فريق المستثمر
     */
    public function index(Request $request): JsonResponse
    {
        $query = TeamMember::with('memberType')
            ->forOwner($request->auth_user_id, 'investor');
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        // Validate member type exists, is active, and belongs to investor scope
        $memberType = MemberType::where('id', $validated['member_type_id'])
            ->active()
            ->where(function ($q) {
                $q->where('scope', 'investor')
                  ->orWhere('scope', 'both');
            })
            ->first();
        ...
        if (!$memberType) {
            return ApiResponse::error(
                'نوع العضو غير صالح أو غير متاح للمستثمرين',
                'INVALID_MEMBER_TYPE',
                422
            );
        }
    ...
    {
        // Ensure the member belongs to this investor
        if ($teamMember->owner_id !== $request->auth_user_id || $teamMember->owner_type !== 'investor') {
    ...
    /**
     * قائمة أنواع الأعضاء المتاحة للمستثمرين
     */
