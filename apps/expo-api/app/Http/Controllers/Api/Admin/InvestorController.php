...
class InvestorController extends Controller
{
    ...
    {
        $query = BusinessProfile::where('business_type', BusinessType::INVESTOR);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $validated['business_type'] = BusinessType::INVESTOR;
     ...
     * عرض مستثمر
     */
    public function show(BusinessProfile $investor): JsonResponse
    {
        if ($investor->business_type !== BusinessType::INVESTOR) {
            return ApiResponse::notFound('المستثمر غير موجود');
        }

        return ApiResponse::success(new BusinessProfileResource($investor));
    }

    /**
     * تحديث مستثمر
     */
    public function update(Request $request, BusinessProfile $investor): JsonResponse
    {
        if ($investor->business_type !== BusinessType::INVESTOR) {
            return ApiResponse::notFound('المستثمر غير موجود');
        }
        ...
        if (isset($validated['status']) && $validated['status'] === 'approved' && $investor->status->value !== 'approved') {
            $validated['reviewed_by'] = $request->auth_user_id;
            $validated['reviewed_at'] = now();
        }
        ...
        return ApiResponse::success(
            new BusinessProfileResource($investor),
     ...
     * حذف مستثمر
     */
    public function destroy(BusinessProfile $investor): JsonResponse
    {
        if ($investor->business_type !== BusinessType::INVESTOR) {
            return ApiResponse::notFound('المستثمر غير موجود');
        }
        ...
        return ApiResponse::success(null, 'تم حذف المستثمر بنجاح');
    }
}
